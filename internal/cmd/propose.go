...
func proposeCommand() *cobra.Command {
	...
	addNoWebFlag, readNoWebFlag := flags.ProposeNoWeb()
	...
	cmd := cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			noWeb, errNoWeb := readNoWebFlag(cmd)
			stack, errStack := readStackFlag(cmd)
			title, errTitle := readTitleFlag(cmd)
			verbose, errVerbose := readVerboseFlag(cmd)
			if err := cmp.Or(errBodyFile, errBodyText, errDryRun, errNoWeb, errAutoResolve, errStack, errTitle, errVerbose); err != nil {
				return err
			}
...
func proposeProgram(repo execute.OpenRepoResult, data proposeData) program.Program {
	...
	for _, branchToPropose := range data.branchesToPropose {
		...
		if updateBreadcrumb {
			lineageSection := proposallineage.RenderSection(data.config.NormalConfig.Lineage, branchToPropose.name, data.config.NormalConfig.Order, data.config.NormalConfig.ProposalBreadcrumb, data.config.NormalConfig.ProposalBreadcrumbDirection, data.config.NormalConfig.ProposalBreadcrumbStyle, data.connector)
